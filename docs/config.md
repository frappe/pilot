...
| `jwks_url` | string | no | `""` | Optional URL of a remote JWKS endpoint. When set, the admin **also** trusts RS256/384/512 JWTs signed by that issuer's keys, so a remote control plane can log in and drive the API without a shared secret. Inherited by new sibling benches. See [remote login via JWKS](admin.md#remote-login-via-jwks). |
