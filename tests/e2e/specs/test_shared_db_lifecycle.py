"""Full happy-path lifecycle for a development bench on shared system MariaDB
(no ZFS / dedicated DB). Drives the admin UI end to end:
...
To add a variant (e.g. ZFS / dedicated DB): copy this file, change BENCH_NAME /
E2E_DB_MODE, and adjust the wizard call.
"""
...
# ── tunables (override via env to match a heavier matrix later) ────────────────
DB_MODE = os.environ.get("E2E_DB_MODE", "shared")  # 'shared' | 'dedicated'
BENCH_NAME = f"e2e-{DB_MODE}"  # consumed by the `bench` fixture
